...
mysql_select_db("gebruiker") or die(mysql_error()); // Select registration database.
...
if(isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['hash']) && !empty($_GET['hash'])){
    ...
    $search = mysql_query("SELECT email, hash FROM gebruiker WHERE email='".$email."' AND hash='".$hash."' AND active='0'") or die(mysql_error()); 
